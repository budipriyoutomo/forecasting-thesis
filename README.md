...
- **Fase 7** (dashboard & visualisasi) selesai: endpoint `GET /dashboard/summary` (`dashboard_service` agregasi), halaman dashboard (stat tiles + status reorder + run terakhir), `ForecastChart` (tren + confidence interval, recharts), `ExplanationBox` (penjelasan bahasa natural). Coverage backend 95%.
- **Fase 8** (export & laporan) selesai: `export_service` (builder murni xlsx/pdf), endpoint export forecast (Excel) & reorder (Excel/PDF), arsip ke R2 `permanent/exports/` (best-effort), frontend tombol export (unduh blob). Coverage backend 95%.
